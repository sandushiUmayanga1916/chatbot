...
const App = () => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/api/pdf', { story, imageUrl, storyName }, { responseType: 'blob' });
  ...
  const handleRegenerateStory = async (originalStory, regeneratePrompt) => {
    try {
      const response = await axios.post('http://localhost:3001/api/regenerate-story', { story: originalStory, regeneratePrompt });
  ...
  const handleRegenerateImage = async (originalSummary, regeneratePrompt) => {
    try {
      const response = await axios.post('http://localhost:3001/api/regenerate-image', { summary: originalSummary, regeneratePrompt });
  ...
  const handleDescribeImage = async (imageUrl) => {
    try {
      const response = await axios.post('http://localhost:3001/api/describe-image', { imageUrl });
